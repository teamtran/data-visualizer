...
class ElectricalPlot:
    ...
    def _find_linear_regime_from_up_transfer_curve(
        self, threshold: float = 5, sampling_rate: int = 5
    ) -> tuple:
        """
    ...
    def plot_transfer_curve(
        ...
        ax.plot(
            self.data["GateV"][0 : self.up_down_idx],
            self.data["sqrt_DrainI"][0 : self.up_down_idx],
            color=self.style["color"]["blue"],
            label="id_up",
            linewidth=1.5,
            markersize=12,
            linestyle="dotted",
        ...
        ax.plot(
            self.data["GateV"][self.up_down_idx :],
            self.data["sqrt_DrainI"][self.up_down_idx :],
            color=self.style["color"]["yellow"],
            label="id_down",
            linewidth=1.5,
            markersize=12,
            linestyle="dotted",
        ...
        ax2.plot(
            self.data["GateV"][0 : self.up_down_idx],
            np.abs(self.data["DrainI"][0 : self.up_down_idx]),
            color=self.style["color"]["blue"],
        ...
        ax2.plot(
            self.data["GateV"][self.up_down_idx :],
            np.abs(self.data["DrainI"][self.up_down_idx :]),
            color=self.style["color"]["yellow"],
            label="$\mathregular{I_{D}}$, down",
            linewidth=1.5,
            markersize=12,
        )
        ax2.set_xlabel("Gate Voltage, $\mathregular{V_{G}}$ (V)")  # x-axis label
    ...
    def export_metadata(self):
        ...
        NOTE: only use this once you've plotted everything
        ...
        id_on, id_off, id_on_off = self._calculate_id_on_off_ratio(
            self.max_x_range, self.min_x_range
        )
