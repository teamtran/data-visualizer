...
def calculate_mobility(slope_DrainI: float, slope_id_up: float):
    """Function that calculates the mobility of the OFET."""
    u_down: float = (2 * l * (np.abs(slope_DrainI) ** 2)) / (c * w)
...
def plot_transfer_curves(filenames: list):
    ...
    for filename in filenames:
        ...
        slope_DrainI, intercept, r_value, p_value, std_err = stats.linregress(
            preprocessed_data["GateV"][idx0:idx1],
            preprocessed_data["sqrt_DrainI"][idx0:idx1],
        )
        print(slope_DrainI, intercept, r_value, p_value, std_err)
        # get r2 of the fitted curve
        r2 = r2_score(
            preprocessed_data["sqrt_DrainI"][idx0:idx1],
            slope_DrainI * preprocessed_data["GateV"][idx0:idx1] + intercept,
        )
        print(r2)
        # plot fitted curve
        ax.plot(
        ...
        ax.text(
            preprocessed_data["GateV"][idx0],
            preprocessed_data["sqrt_DrainI"][idx0] - 1e-5,
            "Slope: {:.5e}".format(slope_DrainI),
            fontsize=12,
        )
        ax.text(
            preprocessed_data["GateV"][idx0],
            preprocessed_data["sqrt_DrainI"][idx0] - 1e-5 - 2e-6,
            "Intercept: {:.5e}".format(intercept),
            fontsize=12,
        )
        ax.text(
            preprocessed_data["GateV"][idx0],
            preprocessed_data["sqrt_DrainI"][idx0] - 1e-5 - 4e-6,
            "R$^2$: {:.5f}".format(r2),
            fontsize=12,
        )
        ...
        # calculate mobility
        u_down, u_up, u_average = calculate_mobility(slope_DrainI, slope_id_up)
